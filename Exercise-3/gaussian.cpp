...
struct GaussianCostFunction
{
	...
	bool operator()(const T* const mu, const T* const sigma, T* residual) const
	{
		...
		T x = T(point.x);
		T y_observed = T(point.y);

		T denom = T(sqrt(2.0 * M_PI)) * sigma[0];
		T exponent = - (x - mu[0]) * (x - mu[0]) / (T(2.0) * sigma[0] * sigma[0]);
		T y_pred = ceres::exp(exponent) / denom;

		residual[0] = y_pred - y_observed;
		return true;
	}
